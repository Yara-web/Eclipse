...
//! Для медленного скролла страницы при нажатии на ссылки
...
/* function app() {
	const buttons = document.querySelectorAll('.filter-nav__item')
	const cards = document.querySelectorAll('.filter-body__item')

	function filter (category, items) {
		items.forEach ((item) => {
			const isItemFiltered = !item.classList.contains(category)
			const isShowAll = category.toLowerCase() === 'development'
...
//! Фильтр на JQuery с вешанием класса active

$('.filter-nav__item').click(function(event){
	var i=$(this).data('filter');
	if (i==1) {
		$('.filter-body__item').show();
	}else{
		$('.filter-body__item').hide();
		$('.filter-body__item.f_'+i).show();
	}//! Добавляэт класс active
